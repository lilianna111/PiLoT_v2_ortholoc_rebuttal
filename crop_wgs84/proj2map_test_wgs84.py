...
def geodetic_to_ecef(lon, lat, h):
    x, y, z = _WGS84_TO_ECEF.transform(lon, lat, h)
    return np.array([x, y, z], dtype=np.float64)


def ecef_to_geodetic(point):
    x, y, z = np.asarray(point, dtype=np.float64)[:3]
    lon, lat, h = _ECEF_TO_WGS84.transform(float(x), float(y), float(z))
    return float(lon), float(lat), float(h)


def _enu_axes_from_lonlat(lon, lat):
    lon_rad = np.deg2rad(lon)
    lat_rad = np.deg2rad(lat)
    east = np.array([-np.sin(lon_rad), np.cos(lon_rad), 0.0], dtype=np.float64)
    north = np.array([-np.sin(lat_rad) * np.cos(lon_rad), -np.sin(lat_rad) * np.sin(lon_rad), np.cos(lat_rad)], dtype=np.float64)
    up = np.array([np.cos(lat_rad) * np.cos(lon_rad), np.cos(lat_rad) * np.sin(lon_rad), np.sin(lat_rad)], dtype=np.float64)
    return east, north, up


def ecef_to_enu(points, ref_point_ecef=None, ref_lon=None, ref_lat=None):
    pts = np.asarray(points, dtype=np.float64)
    squeeze = pts.ndim == 1
    if squeeze:
        pts = pts[None, :]
    if ref_point_ecef is None:
        ref_point_ecef = pts[0]
    ref_point_ecef = np.asarray(ref_point_ecef, dtype=np.float64)
    if ref_lon is None or ref_lat is None:
        ref_lon, ref_lat, _ = ecef_to_geodetic(ref_point_ecef)
    east, north, up = _enu_axes_from_lonlat(ref_lon, ref_lat)
    delta = pts - ref_point_ecef[None, :]
    enu = np.stack([delta @ east, delta @ north, delta @ up], axis=-1)
    return enu[0] if squeeze else enu


def enu_to_ecef(enu_points, ref_point_ecef, ref_lon=None, ref_lat=None):
    enu = np.asarray(enu_points, dtype=np.float64)
    squeeze = enu.ndim == 1
    if squeeze:
        enu = enu[None, :]
    ref_point_ecef = np.asarray(ref_point_ecef, dtype=np.float64)
    if ref_lon is None or ref_lat is None:
        ref_lon, ref_lat, _ = ecef_to_geodetic(ref_point_ecef)
    east, north, up = _enu_axes_from_lonlat(ref_lon, ref_lat)
    ecef = ref_point_ecef[None, :] + enu[:, [0]] * east[None, :] + enu[:, [1]] * north[None, :] + enu[:, [2]] * up[None, :]
    return ecef[0] if squeeze else ecef


def pose_w2c_to_c2w(pose_w2c):
    R = pose_w2c[:3, :3]
    t = pose_w2c[:3, 3]
...
def pixel_to_world(K, pose_w2c, uv, target_z):
    """
    在 ECEF 中计算像素射线与局部切平面(geodetic height = target_z)的交点。
    这里不再假设全局 z = 常数，而是在相机当前位置的局部 Up 方向上定义平面。
    """
    c2w = pose_w2c_to_c2w(pose_w2c)
    cam_center = c2w[:3, 3]
    ray_dir = pixel_to_ray_dir(K, pose_w2c, uv)

    cam_lon, cam_lat, _ = ecef_to_geodetic(cam_center)
    east, north, up = _enu_axes_from_lonlat(cam_lon, cam_lat)
    plane_origin = geodetic_to_ecef(cam_lon, cam_lat, target_z)

    denom = float(np.dot(ray_dir, up))
    if abs(denom) < 1e-10:
        return plane_origin.copy()

    scale = float(np.dot(plane_origin - cam_center, up) / denom)
    if scale <= 0:
        scale = 1.0
...
def geo_coords_to_dsm_index(point_or_x, y_or_transform, transform=None):
    if transform is None:
        point = np.asarray(point_or_x, dtype=np.float64)
        transform = y_or_transform
    else:
        point = np.array([point_or_x, y_or_transform], dtype=np.float64)

    if point.size >= 3:
        lon, lat, _ = ecef_to_geodetic(point[:3])
    else:
...
def dsm_index_to_geodetic(row, col, transform, h=None):
    lon = transform.c + col * transform.a + row * transform.b
    lat = transform.f + col * transform.d + row * transform.e
    return lon, lat, h


def build_points_3d_for_crop(row_min, row_max, col_min, col_max, dsm_crop, dsm_transform):
    rows = np.arange(row_min, row_max, dtype=np.float64)
    cols = np.arange(col_min, col_max, dtype=np.float64)
    cc, rr = np.meshgrid(cols, rows)
    lon = dsm_transform.c + cc * dsm_transform.a + rr * dsm_transform.b
    lat = dsm_transform.f + cc * dsm_transform.d + rr * dsm_transform.e
    zz = np.nan_to_num(dsm_crop, nan=0.01).astype(np.float64, copy=False)
    xx, yy, zz_ecef = _WGS84_TO_ECEF.transform(lon, lat, zz)
    return np.stack([xx, yy, zz_ecef], axis=-1)


def ray_intersect_dsm(ray_origin, ray_dir, dsm_data, dsm_transform, max_distance=10000, step=1.0):
    ...
    for i in range(num_samples):
        ...
        row, col = geo_coords_to_dsm_index(point, dsm_transform)
        if 0 <= row < dsm_data.shape[0] and 0 <= col < dsm_data.shape[1]:
            dsm_h = dsm_data[row, col]
            if np.isnan(dsm_h) or dsm_h <= 0:
                continue
            lon, lat, point_h = ecef_to_geodetic(point)
            if point_h <= dsm_h + 0.1:
                if dsm_h > best_height:
                    best_height = dsm_h
                    best_point = geodetic_to_ecef(lon, lat, dsm_h)
    return best_point
...
def get_height_from_center_pixel(K, pose_w2c, center_point, dsm_data, dsm_transform):
    ...
    if intersect_point is not None:
        _, _, height = ecef_to_geodetic(intersect_point)
...
def crop_dsm_dom(dom_data, dsm_data, dsm_transform,
    ...
    for uv in image_points:
        ...
        print('----------',xyz,'--------------')
        idx = geo_coords_to_dsm_index(xyz, dsm_transform)
    ...
    if current_height > max_crop_size or current_width > max_crop_size:
        ...
        center_row, center_col = geo_coords_to_dsm_index(center_xyz, dsm_transform)
    ...
    points_3d = build_points_3d_for_crop(row_min, row_max, col_min, col_max, dsm_crop, dsm_transform)
...
def order_quad_points_clockwise(pts):
    """输入四边形顶点，返回 [tl, tr, br, bl] 顺序。"""
    pts = np.asarray(pts, dtype=np.float32)
    if pts.shape != (4, 2):
        raise ValueError(f"order_quad_points_clockwise expects (4,2), got {pts.shape}")
...
def compute_rectified_size(src_quad):
    """根据四边形四条边长度，估计透视转正后的宽高。"""
    tl, tr, br, bl = src_quad
    width_top = np.linalg.norm(tr - tl)
    width_bottom = np.linalg.norm(br - bl)
    height_left = np.linalg.norm(bl - tl)
    height_right = np.linalg.norm(br - tr)
    W = max(int(round(max(width_top, width_bottom))), 2)
    H = max(int(round(max(height_left, height_right))), 2)
    return W, H


def warp_dom_and_pointcloud(dom_crop, point_cloud_crop, src_vertices_xy):
    """
    对 DOM 和对应 ECEF 点云使用同一个单应性做透视转正，保证 2D / 3D 对齐。
    返回: warped_dom_chw, warped_points_xyz, warped_mask
    """
    if dom_crop.ndim == 3 and dom_crop.shape[0] == 3:
        img = np.transpose(dom_crop, (1, 2, 0)).copy()
        need_back_chw = True
    ...
    img = np.clip(img, 0, 255).astype(np.uint8)
    src = order_quad_points_clockwise(np.asarray(src_vertices_xy, dtype=np.float32))
    W, H = compute_rectified_size(src)
    ...
    warped_points = np.empty((H, W, 3), dtype=np.float64)
    for c in range(3):
        warped_points[:, :, c] = cv2.warpPerspective(
            point_cloud_crop[:, :, c].astype(np.float64),
            H_mat, (W, H),
            flags=cv2.INTER_LINEAR,
            borderMode=cv2.BORDER_CONSTANT,
            borderValue=np.nan,
    ...
    return warped_img, warped_points, warped_mask


def crop_to_valid_mask(dom_crop_rectified, point_cloud_rectified, mask_rectified, white_bg=True):
    """按 mask 的有效区域裁边，继续保持 DOM / 点云严格对齐。"""
    ys, xs = np.where(mask_rectified > 0)
    if len(xs) == 0 or len(ys) == 0:
        return dom_crop_rectified, point_cloud_rectified, mask_rectified

    x_min, x_max = int(xs.min()), int(xs.max()) + 1
    y_min, y_max = int(ys.min()), int(ys.max()) + 1

    if dom_crop_rectified.ndim == 3 and dom_crop_rectified.shape[0] == 3:
        dom_out = dom_crop_rectified[:, y_min:y_max, x_min:x_max].copy()
        if white_bg:
            img_hwc = np.transpose(dom_out, (1, 2, 0))
            img_hwc[mask_rectified[y_min:y_max, x_min:x_max] == 0] = 255
            dom_out = np.transpose(img_hwc, (2, 0, 1))
    else:
        dom_out = dom_crop_rectified[y_min:y_max, x_min:x_max].copy()
        if white_bg:
            dom_out[mask_rectified[y_min:y_max, x_min:x_max] == 0] = 255

    pts_out = point_cloud_rectified[y_min:y_max, x_min:x_max, :].copy()
    mask_out = mask_rectified[y_min:y_max, x_min:x_max].copy()
    return dom_out, pts_out, mask_out
...
def find_short_parallel_edge(world_points, parallel_threshold=0.15):
    """
    在局部 ENU 平面中找到梯形两条平行边中较短的那条边。
    输入 world_points 为 ECEF。
    """
    if len(world_points) < 4:
        return None

    pts = np.asarray(world_points, dtype=np.float64)
    ref_point = np.mean(pts, axis=0)
    ref_lon, ref_lat, _ = ecef_to_geodetic(ref_point)
    pts_enu = ecef_to_enu(pts, ref_point_ecef=ref_point, ref_lon=ref_lon, ref_lat=ref_lat)

    edges = []
    for i in range(4):
        p1 = np.array(pts_enu[i][:2], dtype=np.float64)
        p2 = np.array(pts_enu[(i + 1) % 4][:2], dtype=np.float64)
        edge_vector = p2 - p1
        edge_length = np.linalg.norm(edge_vector)
        if edge_length > 0:
    ...
    best_pair = max(parallel_pairs, key=lambda x: x[0])
    _, edge1_idx, edge2_idx, len1, len2 = best_pair
    edge_idx = edge1_idx if len1 < len2 else edge2_idx
    _, direction, length, start_idx, end_idx = edges[edge_idx]
    return (float(length), np.asarray(direction, dtype=np.float64), start_idx, end_idx)
...
def build_rotated_rectangle_from_center_and_edge(center_world, edge_length_along_short_direction,
    ...
    """
    基于中心点 ECEF、局部 ENU 中的短边方向与长度构建斜向长方形，输出 ECEF 顶点。
    """
    center_world = np.asarray(center_world, dtype=np.float64)
    center_lon, center_lat, _ = ecef_to_geodetic(center_world)
    ...
    rect_enu = np.array([
        [ half_u * u[0] + half_v * v[0],  half_u * u[1] + half_v * v[1], 0.0],
        [ half_u * u[0] - half_v * v[0],  half_u * u[1] - half_v * v[1], 0.0],
        [-half_u * u[0] - half_v * v[0], -half_u * u[1] - half_v * v[1], 0.0],
        [-half_u * u[0] + half_v * v[0], -half_u * u[1] + half_v * v[1], 0.0],
    ], dtype=np.float64)

    rect_ecef = enu_to_ecef(rect_enu, ref_point_ecef=center_world, ref_lon=center_lon, ref_lat=center_lat)
    return [p for p in rect_ecef]
    ...
    for name in query_intrinsics.keys():
        ...
        if debug and crop_vis_save_path:
            ...
            try:
                ...
                row_ml, col_ml = geo_coords_to_dsm_index(world_left_mid, dsm_transform)
                row_mr, col_mr = geo_coords_to_dsm_index(world_right_mid, dsm_transform)
                ...
                pt_ml1 = (int(round(ml_x1)), int(round(ml_y1)))
                pt_ml2 = (int(round(ml_x2)), int(round(ml_y2)))
...
def generate_ref_map(
    name, query_intrinsics, query_poses, area_minZ,
    dsm_data, dsm_transform, dom_data,
    ref_rgb_path=None, ref_depth_path=None,
    debug=False, crop_vis_save_path=None,
    test_ratio=None, use_bbox=True,
    save_to_disk=False,
):
    """
    生成参考 crop。默认只返回内存中的数组，不保存 jpg/npy。

    参数:
        test_ratio: 仅在 use_bbox=False 时使用。
        use_bbox: True 时使用中心点投影 + 短边构建斜向长方形裁剪。
        save_to_disk: True 时额外保存 DOM / 点云 / mask 到磁盘。
    """
    ...
    crop_mask_final = None

    if use_bbox and rotated_rect_vertices_relative is not None:
        dom_crop_rectified, point_cloud_rectified, crop_mask_rectified = warp_dom_and_pointcloud(
            dom_crop, point_cloud_crop, rotated_rect_vertices_relative
        )
        dom_crop, point_cloud_crop, crop_mask_final = crop_to_valid_mask(
            dom_crop_rectified, point_cloud_rectified, crop_mask_rectified, white_bg=True
        )

    if save_to_disk:
        if ref_rgb_path is None or ref_depth_path is None:
            raise ValueError("save_to_disk=True 时，ref_rgb_path 和 ref_depth_path 不能为 None")
        ...
        ratio_img_uint8 = np.clip(dom_crop, 0, 255).astype(np.uint8)
        if ratio_img_uint8.ndim == 3 and ratio_img_uint8.shape[0] == 3:
            ratio_img_uint8 = np.transpose(ratio_img_uint8, (1, 2, 0))
        Image.fromarray(ratio_img_uint8).save(ratio_img_path)
        np.save(ratio_npy_path, point_cloud_crop)
        if crop_mask_final is not None:
            cv2.imwrite(ratio_mask_path, crop_mask_final)
        print(f"    💾 已保存: {ratio_img_path}")
    else:
        print("    ✅ crop 已生成（仅内存返回，未保存到磁盘）")

    ref_h = dom_crop.shape[1]
    ref_w = dom_crop.shape[2] if dom_crop.ndim == 3 else dom_crop.shape[1]
    ref_total_area = ref_w * ref_h
    ...
    if debug and crop_vis_save_path:
        ...
        for idx, (x, y) in enumerate(ref_indices):
            ...
            if 0 <= x < ref_width and 0 <= y < ref_height:
                ...
                near_x = np.clip(x, 0, ref_width - 1)
                near_y = np.clip(y, 0, ref_height - 1)
                cv2.drawMarker(ref_vis, (int(near_x), int(near_y)), color=(255, 255, 0),
        ...
        if rotated_rect_vertices_relative is not None:
            ...
            print("  Debug: rotated_rect_vertices_relative 为 None，不绘制斜向长方形")
        ...
        try:
            ...
            label_x = (pt_ml1[0] + pt_ml2[0]) // 2
            label_y = (pt_ml1[1] + pt_ml2[1]) // 2 - 15
            ts, bl = cv2.getTextSize(mid_label, cv2.FONT_HERSHEY_SIMPLEX, adaptive_font_scale, adaptive_thickness + 1)
            tw_ml, th_ml = ts
            overlay_ml = ref_vis.copy()
            cv2.rectangle(overlay_ml, (label_x - tw_ml // 2 - 4, label_y - th_ml - 4),
                          (label_x + tw_ml // 2 + 4, label_y + bl + 4), (0, 0, 0), -1)
            cv2.addWeighted(overlay_ml, 0.6, ref_vis, 0.4, 0, ref_vis)
            cv2.putText(ref_vis, mid_label, (label_x - tw_ml // 2, label_y),
                        cv2.FONT_HERSHEY_SIMPLEX, adaptive_font_scale,
                        (255, 255, 255), adaptive_thickness + 1, cv2.LINE_AA)
        except Exception as _ml_err:
            print(f"    ⚠️ 绘制中线失败: {_ml_err}")

        ratio_text = f"RotRect A:{actual_ratio_percent:.1f}%" if (use_bbox or test_ratio is None) else f"T:{test_ratio*100:.0f}% A:{actual_ratio_percent:.1f}%"
        text_size, baseline = cv2.getTextSize(ratio_text, cv2.FONT_HERSHEY_SIMPLEX, adaptive_font_scale, adaptive_thickness + 1)
        text_width, text_height = text_size
        overlay = ref_vis.copy()
        cv2.rectangle(overlay, (5, 5), (text_width + 15, text_height + baseline + 20), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.6, ref_vis, 0.4, 0, ref_vis)
        cv2.putText(ref_vis, ratio_text, (10, 25), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=adaptive_font_scale, color=(255, 255, 255),
                    thickness=adaptive_thickness + 1, lineType=cv2.LINE_AA)
    ...
    return {
        ...
        'crop_mask': crop_mask_final,
        'min_shape': min_shape,
        'dsm_indices': dsm_indices,
        'world_coords': world_coords,
        'crop_offset': crop_offset,
        'rotated_rect_vertices_relative': rotated_rect_vertices_relative,
        'actual_ratio_percent': actual_ratio_percent,
        'saved_paths': {
            'rgb': ratio_img_path,
            'depth': ratio_npy_path,
            'mask': ratio_mask_path,
        },
    }
...
def crop_dsm_dom_angle(dom_data, dsm_data, dsm_transform,
    ...
    if target_ratio is None:
        ...
        if short_edge_info is None:
            ...
            center_xyz = pixel_to_world(K, pose_w2c, center_point, target_z=area_minZ)
            print(f"中心点投影（ECEF）: ({center_xyz[0]:.2f}, {center_xyz[1]:.2f}, {center_xyz[2]:.2f})")
            ...
            for i, v in enumerate(rect_vertices):
                print(f"  顶点{i+1}: ({v[0]:.2f}, {v[1]:.2f}, {v[2]:.2f})")

            # 步骤4: 将长方形顶点转换为DSM像素坐标
            rotated_rect_pixel_coords = []
            for vertex in rect_vertices:
                row, col = geo_coords_to_dsm_index(vertex, dsm_transform)
