...
CONFIG_PATH="${SCRIPT_DIR}/configs/feicuiwan_m4t_uavscene.yaml"
UAVSCENE_K_JSON="${SCRIPT_DIR}/OrthoLoC/OrthoLoC/crop/K_uavscene.json"
CROP_K_JSON="${CROP_K_JSON:-${UAVSCENE_K_JSON}}"
ORTHOLOC_INTRINSICS="${ORTHOLOC_INTRINSICS:-${UAVSCENE_K_JSON}}"

DOM_PATH="${DOM_PATH:-/media/amax/AE0E2AFD0E2ABE69/datasets/uavscene/model/DOMDSM/HKairport/dom_wgs84.tif}"
DSM_PATH="${DSM_PATH:-/media/amax/AE0E2AFD0E2ABE69/datasets/uavscene/model/DOMDSM/HKairport/dsm_wgs84.tif}"

ORTHOLOC_MATCHER="${ORTHOLOC_MATCHER:-Mast3R}"
ORTHOLOC_DEVICE="${ORTHOLOC_DEVICE:-cuda}"
ORTHOLOC_ANGLE_ARGS=()
if [[ -n "${ORTHOLOC_ANGLES:-}" ]]; then
  read -r -a ortholoc_angles <<< "$ORTHOLOC_ANGLES"
  ORTHOLOC_ANGLE_ARGS=(--ortholoc_angles "${ortholoc_angles[@]}")
fi
...
for target_name in "${target_names[@]}"; do
  ...
  if [[ $index -ge 0 ]]; then
    if ! result=$(read_pose_from_file "$target_name"); then
