...
def process_map_crop(ref_DSM_path, pose_data, ref_npy_path, name, map_data_pack, paths_pack, ray_area, ray_area_minZ):        # 包含输出路径
    ...
    #     debug=False
    # )
    data = generate_ref_map(name, query_intrinsics_dict, query_prior_poses_dict, area_minZ, dsm_data, dsm_trans, dom_data, 
    ref_rgb_path, ref_depth_path, debug=False, crop_vis_save_path=None, test_ratio=None, use_bbox=True)

    # 加载 Stage 1 生成的文件
    img_aabb = cv2.imread(os.path.join(ref_rgb_path, f"{name}_bbox_dom.jpg"))
    mask_aabb = cv2.imread(os.path.join(ref_rgb_path, f"{name}_bbox_dom_mask.png"), cv2.IMREAD_GRAYSCALE)
    npy_aabb = np.load(os.path.join(ref_rgb_path, f"{name}_bbox_dom.npy")) # 注意加载正确路径
...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        output_folder = "/media/amax/AE0E2AFD0E2ABE69/datasets/outputs/FPVLoc_depth_angle_crop"
    ...
    def rendering_worker(self):
        ...
        while True:
            ...
            tt0 = time.time()
            try:
                final_img, final_npy, final_mask = process_map_crop(
                    ref_DSM_path,
                    pose_data, 
                    ref_npy_path, 
                    name, 
            ...
            if final_img is None or final_npy is None or final_mask is None:
                logging.error(f"裁剪结果为空 name={name}")
                continue
            # 使用返回值：final_img 按 OpenCV 为 BGR，转 RGB
            color = np.asarray(final_img)
            if color.ndim == 3 and color.shape[-1] == 3:
                color = cv2.cvtColor(color, cv2.COLOR_BGR2RGB)
            points3d = np.asarray(final_npy, dtype=np.float32)
            # 有效区域以 final_mask 为准（squeeze 避免 (H,W,1) 导致索引错误）
            valid_mask = np.asarray(final_mask, dtype=bool).squeeze()
            if valid_mask.ndim != 2 or valid_mask.shape != color.shape[:2] or valid_mask.shape != points3d.shape[:2]:
                logging.error(f"裁剪结果尺寸不一致 name={name}: img={color.shape}, npy={points3d.shape}, mask={valid_mask.shape}")
                continue
            if not np.any(valid_mask):
                logging.error(f"裁剪结果无有效区域( mask 全为假) name={name}")
                continue
            ...
            # mask 有效区域同步 resize（最近邻保持二值）
            mask = valid_mask.astype(np.uint8)
            mask = cv2.resize(mask, (target_w, target_h), interpolation=cv2.INTER_NEAREST)

            # padding 到 16 的倍数，使用复制边界；只对 mask 有效区域内的数据有意义
            if self.padding:
            ...
            # 只在 mask 有效区域内采样 3D 点
            points_valid = points3d[mask]
            if points_valid.shape[0] == 0:
                logging.error(f"裁剪结果无有效3D点(经过 resize/padding 后 mask 内无点) name={name}")
                continue
