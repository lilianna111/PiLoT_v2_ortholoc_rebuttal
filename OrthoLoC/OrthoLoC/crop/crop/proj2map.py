...
def generate_ref_map(
    ...
    t_near: float = 1.0,
    t_far: float = 20000.0,
    cached_dsm_array: np.ndarray | None = None,
    cached_dsm_transform: Affine | None = None,
    cached_dsm_nodata=None,
    cached_dom_array: np.ndarray | None = None,
):
    t0_total = time.perf_counter()
    K = normalize_K(K)

    t0 = time.perf_counter()
    if cached_dsm_array is None or cached_dsm_transform is None or cached_dom_array is None:
        dsm_array, dsm_transform, _, dsm_nodata, dom_array, _, _ = read_dsm_dom(dsm_path, dom_path)
    else:
        dsm_array = cached_dsm_array
        dsm_transform = cached_dsm_transform
        dsm_nodata = cached_dsm_nodata
        dom_array = cached_dom_array
