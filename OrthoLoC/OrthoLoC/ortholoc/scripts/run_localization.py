...
def run_localization(matcher_name: str, img_path: str | None = None, dop_path: str | None = None,
                     ...
                     plot_max_pts: int = 1000, fig_ext: str = '.png', matcher: MatcherIMCUI | MatcherGT | None = None,
                     cached_dop: np.ndarray | None = None, cached_dsm: np.ndarray | None = None,
                     save_figures: bool = True, crop_match_plot_to_dom_pts: bool = False,
                     crop_match_plot_margin_ratio: float = 0.15) -> None:
    """
    ...
    if img_path is not None and dop_path is not None and dsm_path is not None:
        ...
        image_dop = cached_dop if cached_dop is not None else utils.io.load_dop_tif(dop_path)
        dsm = cached_dsm if cached_dsm is not None else utils.io.load_dsm_tif(dsm_path)
        ...
        if success:
            ...
            if use_adhop:
                ...
                if success_refined:
                    ...
                    if (output_dir is not None and save_figures) or show:
                        ...
                        fig_matches, _ = correspondences_2d2d.plot(
                            ...
                            crop_img2_to_pts=crop_match_plot_to_dom_pts,
                            crop_margin_ratio=crop_match_plot_margin_ratio)
