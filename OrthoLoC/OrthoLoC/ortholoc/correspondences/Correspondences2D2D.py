...
class Correspondences2D2D(Correspondences):
    ...
    def plot(self, img1: np.ndarray, img2: np.ndarray, img1_name: str | None = None, img2_name: str | None = None,
             ...
             marker: str = 'o', fig_scale: float = 0.5, dpi: int = 100, crop_img2_to_pts: bool = False,
             crop_margin_ratio: float = 0.15) -> tuple[plt.Figure, list[plt.Axes]]:
        """
        ...
        if crop_img2_to_pts and len(positions_2) > 0:
            min_xy = np.floor(np.nanmin(correspondences_2d2d.pts1, axis=0)).astype(int)
            max_xy = np.ceil(np.nanmax(correspondences_2d2d.pts1, axis=0)).astype(int)
            span_xy = np.maximum(max_xy - min_xy, 1)
            margin_xy = np.maximum((span_xy * crop_margin_ratio).astype(int), 32)

            x0 = max(0, min_xy[0] - margin_xy[0])
            y0 = max(0, min_xy[1] - margin_xy[1])
            x1 = min(w1, max_xy[0] + margin_xy[0] + 1)
            y1 = min(h1, max_xy[1] + margin_xy[1] + 1)

            if x1 > x0 and y1 > y0:
                img2 = img2[y0:y1, x0:x1]
                positions_2 = positions_2 - np.array([x0, y0], dtype=positions_2.dtype)
