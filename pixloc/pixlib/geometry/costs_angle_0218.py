...
class DirectAbsoluteCost2:
    def __init__(self):
        ...
        # 关闭深度约束（DSM prior）。如需启用再改回 True。
        self.enable_depth_prior = False
        # 关闭角度约束（roll/pitch prior）。开 True 时用固定 scale
        # 如果加入角度约束后误差反而增大，可能是角度定义不一致或与重投影冲突，可临时设为 False 验证
    ...
    def residual_jacobian_batch_quat(
        ...
        # === Step 5: 计算（可选的）先验约束：角度 prior / 深度 prior（前置计算）===
        # 这里的“先验”指额外的约束项，用来在视觉匹配不稳定时给优化一个温和的引导。
        # 本文件当前只启用了角度 prior（roll/pitch），深度 prior 已被禁用（见 __init__）。
        angle_res = None
        J_angle_tensor = None
        # angle_weight：与后面的 angle_scale 共同决定强度。设为 1.0 时，最终强度 = angle_scale（否则会多乘 3 倍导致拉过头、结果比纯投影还差）
        angle_weight = 1.0

        # === Step 6: Angle prior (adaptive scaling), depth prior disabled ===
        # 深度约束停用：depth_res / J_depth_tensor / valid_depth 会一直保持 None。
        #
        # 角度 prior 的目标：
        ...
        if angle_res is not None:
            # 角度约束的 cost (roll^2 + pitch^2)，弧度²
            cost_angle = (angle_res ** 2).sum(-1, keepdim=True)  # [1, num_init_pose, 1]
            # 拼接 cost，统一计算鲁棒核权重
            cost_all = torch.cat([cost_proj_for_selection, cost_angle], dim=-1)  # [1, num_init_pose, N+1]
            cost_all, w_loss_all, _ = self.loss_fn1(cost_all)
            w_loss_proj = w_loss_all[..., :-1]    # [1, num_init_pose, N]
            w_loss_angle = w_loss_all[..., -1:]   # [1, num_init_pose, 1]
            # 返回的 cost：投影用鲁棒核处理后的值，角度也用鲁棒核处理后的值（量纲一致）
            # 角度项用于最终候选选择时，按有效点数缩放到与重投影可比量级。
            cost_proj_robust = cost_all[..., :-1] * valid_norm
            cost_angle_robust = cost_all[..., -1:]  # [1, num_init_pose, 1]，经过鲁棒核的角度 cost
            cost = torch.cat([
                cost_proj_robust,  # 投影 cost（鲁棒核处理后 + 有效点数归一化）
                float(self.angle_selection_weight) * valid_count * cost_angle_robust
            ], dim=-1)  # [1, num_init_pose, N+1]
        else:
        ...
        # === 角度约束的梯度和 Hessian（如果存在） ===
        # 用固定小 scale，避免 LM 跑飞；角度主要靠“选 pose 时 cost 含角度”起作用。
        if angle_res is not None and w_loss_angle is not None:
            ...
            # 关键：角度梯度量级远小于重投影（log 中 ratio 常在 1e-3），
            # 使用梯度范数比做自适应放大，让角度项在 LM 中真正“有力”。
            eps_bal = 1e-12
            gn_proj = grad_proj.norm(dim=-1, keepdim=True)      # [1,N,1]
            gn_angle = grad_angle.norm(dim=-1, keepdim=True)    # [1,N,1]
            ...
            # 角度残差越大，越降低角度项影响，避免错误先验把优化拉偏。
            # gate=1: 基本信任角度先验；gate≈angle_gate_min: 仅保留很弱约束。
            ...
            if self.angle_debug_print:
                angle_res_deg = torch.rad2deg(angle_res.norm(dim=-1))
                ard = angle_res_deg.detach().cpu().numpy().ravel()
                np_proj = grad_proj.norm(dim=-1).detach().cpu().numpy().ravel()
                np_angle = grad_angle.norm(dim=-1).detach().cpu().numpy().ravel()
                ratio = np_angle / (np_proj + 1e-12)
                np_gate = gate.detach().cpu().numpy().ravel()

                # 诊断1：角度残差是否在迭代中下降（同一次 run 内）
                mean_deg = float(ard.mean())
                if self._angle_prev_mean_deg is None:
                    trend_msg = "start"
                else:
                    delta_deg = mean_deg - self._angle_prev_mean_deg
                    trend_msg = f"{delta_deg:+.3f}deg"
                self._angle_prev_mean_deg = mean_deg

                # 诊断2：一阶方向判定（若 dot<0，表示本次线性步会减小角度残差）
                # 用当前总系统 (H, -g) 的近似 GN 步做方向检查。
                try:
                    grad_tmp = grad_proj + grad_angle * angle_scale_per_pose
                    Hess_tmp = Hess_proj + Hess_angle * angle_scale_per_pose.unsqueeze(-1)
                    eye6 = torch.eye(
                        6, device=Hess_proj.device, dtype=Hess_proj.dtype
                    ).view(1, 1, 6, 6)
                    H_damped = Hess_tmp + 1e-6 * eye6
                    delta_dbg = torch.linalg.solve(H_damped, (-grad_tmp).unsqueeze(-1)).squeeze(-1)  # [1, N, 6]
                    J_angle_rot_dbg = J_angle_tensor[..., :3]  # [1, N, 2, 3]
                    pred_dr = torch.einsum("bnij,bnj->bni", J_angle_rot_dbg, delta_dbg[..., :3])  # [1, N, 2]
                    dir_dot = (angle_res * pred_dr).sum(dim=-1)  # [1, N], <0 代表朝 GT 收敛
                    dir_good_ratio = float((dir_dot < 0).float().mean().item())
                    dir_dot_mean = float(dir_dot.mean().item())
                    np_scale = angle_scale_per_pose.detach().cpu().numpy().ravel()
                except Exception:
                    dir_good_ratio = float("nan")
                    dir_dot_mean = float("nan")
                    np_scale = np.array([np.nan], dtype=np.float64)

                print(  # noqa: E501
                    "[angle_prior] res_deg min={:.3f} mean={:.3f} max={:.3f} | "
                    "trend={} | scale_fixed={:.2f} selection_w={:.3f} | "
                    "norm_proj mean={:.4f} norm_angle mean={:.6f} ratio(mean)={:.6f} "
                    "scale_adapt mean={:.3f} max={:.3f} gate mean={:.3f} min={:.3f} | "
                    "dir_good={:.3f} dir_dot_mean={:.6e}".format(
                        float(ard.min()), float(ard.mean()), float(ard.max()),
                        trend_msg,
                        float(self.angle_scale_fixed), float(self.angle_selection_weight),
                        float(np_proj.mean()), float(np_angle.mean()), float(ratio.mean()),
                        float(np_scale.mean()), float(np_scale.max()),
                        float(np_gate.mean()), float(np_gate.min()),
                        dir_good_ratio, dir_dot_mean
                    )
                )
                # Short fixed-prefix line for fast grep in huge logs.
                print(
                    "[ANGLE_W] selection_w={:.3f} scale_fixed={:.2f} "
                    "scale_adapt_mean={:.3f} gate_mean={:.3f}".format(
                        float(self.angle_selection_weight), float(self.angle_scale_fixed),
                        float(np_scale.mean()), float(np_gate.mean())
                    )
                )

            grad_angle_scaled = grad_angle * angle_scale_per_pose  # [1, num_init_pose, 6]
            Hess_angle_scaled = Hess_angle * angle_scale_per_pose.unsqueeze(-1)  # [1, num_init_pose, 6, 6]
            grad = grad_proj + grad_angle_scaled
            Hess = Hess_proj + Hess_angle_scaled
            w_loss = w_loss_proj  # 返回投影部分的 w_loss
        else:
            grad = grad_proj
            Hess = Hess_proj
            w_loss = w_loss_proj
            if self.enable_angle_prior and (gt_roll is not None or gt_pitch is not None) and render_T_ecef is None:
                print("[angle_prior] 未生效: render_T_ecef 为 None，请检查 back_project 是否返回了 render_T_ecef")
